package com.qingyin.cloud.handler;

import com.qingyin.cloud.enums.ErrorEnum;
import com.qingyin.cloud.util.JsonUtils;
...
public class GlobalExceptionHandler implements ErrorWebExceptionHandler {
     ...
     */
    private CommonResponse<?> responseStatusExceptionHandler(ServerWebExchange exchange,
                                                           ResponseStatusException ex) {
        ServerHttpRequest request = exchange.getRequest();
        log.error("[responseStatusExceptionHandler][uri({}/{}) 发生异常]", request.getURI(), request.getMethod(), ex);
        return CommonResponse.failed(ex.getRawStatusCode(), ex.getReason());
    }
    ...
    public CommonResponse<?> defaultExceptionHandler(ServerWebExchange exchange,
                                                   Throwable ex) {
        ServerHttpRequest request = exchange.getRequest();
        log.error("[defaultExceptionHandler][uri({}/{}) 发生异常]", request.getURI(), request.getMethod(), ex);
        // 返回 ERROR CommonResult
        return CommonResponse.failed(ErrorEnum.SYSTEM_ERROR.getCode(), ErrorEnum.SYSTEM_ERROR.getMsg());
    }
